<template>
  <div class="home">
    <!-- banner-->
    <div class="video-box" v-for="(item, index) in bannerData" :key="index">
      <video autoplay muted loop class="video" v-if="item.fileType == 2">
        <source type="video/mp4" :src="item.mediaUrl" />
      </video>
    ...
    <div class="pao-swiper" v-if="swiperData.conData && swiperData.imgData">
        ...
        <div class="swiper-container swiper-con swiper-container-vertical">
          <div class="swiper-wrapper">
            ...
            >
              <div class="slide-box">
                <div class="title-box">
                  <h2 :class="['title', spIndex == index ? 'dong-title' : '']">
                    {{ item.title }}
                  </h2>
                </div>
      ...
      <div class="img-box">
        <div class="swiper-container swiper-img">
          <div class="swiper-wrapper">
            <div
              class="swiper-slide swiper-slide-img-box"
              v-for="(item, index) in swiperData.imgData"
              :key="index"
            >
              <div
                :class="[
                  'swiper-box-img',
                  spIndex == index ? 'dong-swiper-box-img' : ''
                ]"
    ...
    <div class="mi-ka" v-if="pipiData">
      ...
      <div class="img-box">
        ...
        <video
          v-if="pipiData.mediaType == 2"
          controls
          :src="pipiData.mediaUrl"
        ></video>
      </div>
      <div class="content">
        <p class="dec scroll-title">{{ pipiData.content }}</p>
        <div class="con-btn" @click="gotoPage(pipiData)">
          CLICK HERE
...
export default {
  ...
  async asyncData({ app }) {
    let res = await app.$api.article.homeList();
    let swiperData = {
      conData: [],
      imgData: []
    };
    ...
    let pipiData = null;
    // 域名
    // let comUrl = "http://124.222.155.186:3001/svc";
    let comUrl = "/svc";
    if (res.code == 200) {
      // banner
...
.video-box .click-btn:visited,
.video-box .click-btn:link {
  color: #fff;
}
.video-box .tiao {
  position: absolute;
